...
const DEFAULTS = {
  hero: {
    ...
    tagline: 'Where excellence knows no bounds.',
    stat1val: '2018', stat1lbl: 'Founded',
    stat2val: 'PreKG–Class 10', stat2lbl: 'Grades Offered',
    stat3val: '100%', stat3lbl: 'Govt. Recognized',
    ctaText: 'Enquire for Admission'
  },
  admission: {
    show: true, year: '2026–2027',
    phone1: '8088854101', phone2: '7899454101', btnText: 'Apply Now →'
  },
  about: {
    trust: 'Sri Lakshmi Ranganathaswamy Education Trust (R.)',
    founded: '2018', founder: 'Chikkaguddaiah',
  ...
  // ── IMPORTANT: gallery paths use ./gallery/ (root-relative for public pages)
  gallery: [
    ...
    {src:'./gallery/anu1.JPG',                alt:'Annual Day',    cat:'annualday'},
    {src:'./gallery/anu2.JPG',                alt:'Annual Day',    cat:'annualday'},
    {src:'./gallery/anu3.JPG',                alt:'Annual Day',    cat:'annualday'},
    {src:'./gallery/anu4.JPG',                alt:'Annual Day',    cat:'annualday'},
  ],
  announcements: [
    {icon:'🎓', title:'Admissions Open 2026–2027', text:'Seats filling fast! Call 8088854101.', date:'April 2, 2026'}
  ],
  contact: {
    phone1: '8088854101', phone2: '7899454101', whatsapp: '',
    youtube:  'https://www.youtube.com/@SarvothomukhaInternationalpubl/videos',
    facebook: 'https://www.facebook.com/profile.php?id=61572317934375',
    instagram:'https://www.instagram.com/sarvothomukha2018/',
    address:  '#9 1st cross, GajananaNagar, Hegganahalli cross, Bengaluru-560091, Karnataka, India',
    pincode: '', timeStart: '9:30 AM', timeEnd: '4:00 PM', workDays: 'Monday to Saturday'
  },
  settings: {
    ...
    metaDesc:    'Best school in Karnataka.',
    metaKeywords:'school Karnataka, SIPS'
  },
  enquiries: []
};
...
// ── Helper: normalise gallery src paths ──────────────────────
// Admin saves paths as '../gallery/...' (relative to admin/ folder)
// Public pages need './gallery/...' (relative to root)
// This function fixes any path so public pages always work.
function fixGalleryPath(src) {
  if (!src) return src;
  // Turn ../gallery/x  →  ./gallery/x
  return src.replace(/^\.\.\/gallery\//, './gallery/');
}
...
export const SIPS = {
  async get(key) {
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        // Fix gallery paths on the way out
        if (key === 'gallery' && Array.isArray(data)) {
          data = data.map(item => ({ ...item, src: fixGalleryPath(item.src) }));
        }
        _cache[key] = data;
        return _cache[key];
      }
  ...
  // Clear cache for a key so next get() re-fetches from Firestore
  clearCache(key) {
